...
#' with time independent covariates affecting subject's size, note that every covariate in data.frame z
#' only has size effect.
#'
...
#' 
#'  \strong{call.gmusim: } the internal gmusim_size call that produced the object.
#' 
...
#' getGroups, getCovariate and getVarCov can be used to extract some of gmusim_size's components.
...
#' @export
#' @seealso \code{\link[sitar]{sitar}},\code{\link{gmusim}},\code{\link{gmusim_both}},\code{\link{gmusim_com}},\code{\link{gmusim_gr}}.
...
#' resu1 <- gmusim_size(x,y,z,p,n,id,df)
#' summary(resu1)
#' ## Calculate phv, aphv and height at phv (hphv)
#' resu2 <- gmusim_size(x,y,z,p,n,id,df,cal=TRUE)
#' summary(resu2)
#' aphv <- resu2$calindex
#' fitted.values <- resu2$fitted.values
gmusim_size <- function(x, y, z, p, n, id, df, knots, len, cal=FALSE, 
  ...
  if (missing(start)) start <- c(coefpar[c(2:(df + 1))], coefpar[-c(1:(df + 1))], coefpar[1])
  fix <- fixed
  if (!grepl("a", fix)) 
    fix <- paste("a", fix, sep = "+")
  ss <- paste("s", 1:df, sep = "")
  ...
  fitcode <- c("fitenv <- new.env()", "fitenv$fitnlme <- function($pars) {", 
               ...
               "(x - (b)) * exp(c),", "knots=knots, Boundary.knots=bounds))) %*%", "matrix(rep(1,df), ncol=1))", 
  ...
  for (i in c("random", "pars", "fixed", "sscomma", "zcomma", "azcomma", "nsd")) 
  ...
  nlme.out$ns <- coefpar
  if (!"gmusim_size" %in% class(nlme.out)) 
    class(nlme.out) <- c("gmusim_size", class(nlme.out))
  if (cal == TRUE){
